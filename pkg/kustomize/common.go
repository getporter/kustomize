...
//	`yaml` files.
func (m *Mixin) buildAndExecuteKustomizeCmds(ctx context.Context, step interface{}, commands []*exec.Cmd) error {
	...
	for _, kustomizationFile := range kustomization {
		...
		cmd := m.NewCommand(ctx, "kustomize", "build", kustomizationFile, "--reorder", reorder, "-o", manifests+
...
func (m *Mixin) configureGithubToken(ctx context.Context, ghToken string) error {
	if ghToken != "" {
		if m.DebugMode {
		...
		gitCmd := m.NewCommand(ctx, "git", "config", "--global", gitArgs.String(), "https://github.com/")
		if m.DebugMode {
