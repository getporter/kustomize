...
func TestMixin_ValidatePayload(t *testing.T) {
	testcases := []struct {
		...
		{"install.missing-desc", "testdata/bad-install-input.missing-desc.yaml", false, "install.0.kustomize.description: String length must be greater than or equal to 1"},
		{"status.missing-releases", "testdata/bad-status-input.missing-releases.yaml", false, "status.0.kustomize: releases is required"},
	}
